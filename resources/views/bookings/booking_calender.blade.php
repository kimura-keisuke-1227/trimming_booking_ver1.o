...
<div class="container">
    ...
    @endif
    @if ($after_date<=$maxBookingDate) <a href="{{route('booking.selectCalender.salonAndDay' , ['salon' => $salon -> id, 'st_date' => $after_date])}}">次週へ</a>
        @endif
        <table class="table table-striped pc_only">
            <tr>
                <th>日付</th>
                @foreach($days as $day)
                @if ($day <=$maxBookingDate) <th>
                    @php
            ...
            <tr>
                <th>{{$time}}</th>
                @foreach($days as $day)
                @if ($day <=$maxBookingDate) @if($capacities[$day][$timesNum[$time]]> 0)
